/*
   ...
   DIP status: Front tilemap should be complete, sprites are mostly correct, controls
   should be fine.  Protection exists and the PSAC2 background is missing so the
   game is not real playable.  Sound should be correct but isn't working for some reason.

   TODO:
   - protection and other oddities (rungun shows gameplay in attract mode, rungunu doesn't)
   - '936 roz plane
   - sprite palettes may not be entirely right
   - why doesn't sound work (related to protection?  the sound test has a protection hang)
   - fix visrgn
*/
...
static WRITE16_HANDLER( sound_cmd1_w )
{
	if(ACCESSING_LSB) {
		data &= 0xff;
		soundlatch_w(0, data);
		// $$$HACK: don't run timer during the self test
		if (data == 0xfe)
		{
			snd_timer_enable = 0;
		}

		if (!Machine->sample_rate)	/* make selftest pass if sound off */
		{
			if(data == 0xfc || data == 0xfe)
				soundlatch3_w(0, 0x1f);
		}
		return;
	}
}
...
static READ16_HANDLER( sound_status_r )
{
	...
	// HACK:
	// if the timer's disabled and we get a non-hibit response back enable the timer
	if ((!snd_timer_enable) && !(latch & 0x80))
	{
		snd_timer_enable = 1;
	}
...
//	return latch;
	...
	return 0x3f3f;	// makes sound test successful (most games need 1f, violent storm needs 3f)
}
...
static READ16_HANDLER( rngeeprom_r )
{
	if (ACCESSING_LSB)
	{
		return readinputport(1) | EEPROM_read_bit();
	}
...
static MEMORY_WRITE16_START( rngwritemem )
	...
	{ 0x680000, 0x68001f, MWA16_NOP },	// '936 registers
	{ 0x6c0000, 0x6cffff, MWA16_RAM }, 	// PSAC2 ('936) RAM
	{ 0x700000, 0x7007ff, MWA16_RAM },	// "Line RAM" ('936 also?)
...
ROM_START( rungunu )
	...
	ROM_LOAD64_WORD( "247-a11", 0x000000, 0x200000, 0xc3f60854 )
	ROM_LOAD64_WORD( "247-a08", 0x000002, 0x200000, 0x3e315eef )
	ROM_LOAD64_WORD( "247-a09", 0x000004, 0x200000, 0x5ca7bc06 )
	ROM_LOAD64_WORD( "247-a10", 0x000006, 0x200000, 0xa5ccd243 )
	...
	ROM_LOAD( "247-a07", 0x000000, 0x200000, 0x0108142d )
ROM_END
...
GAME( 1993, rungun,   0,      rng, rng, rng, ROT0, "Konami", "Run and Gun (World ver. EAA)")
GAME( 1993, rungunu,  rungun, rng, rng, rng, ROT0, "Konami", "Run and Gun (US ver. UAB)")
GAME( 1993, slmdunkj, rungun, rng, rng, rng, ROT0, "Konami", "Slam Dunk (Japan ver. JAA))")
